...
pub struct GameAssets {
    pub pacman_open: Arc<DynamicImage>,
    pub pacman_closed: Arc<DynamicImage>,
...
impl GameAssets {
    pub fn new(
        pacman_open: DynamicImage,
        pacman_closed: DynamicImage,
        food: DynamicImage,
        empty: DynamicImage,
    ) -> Self {
        Self {
            pacman_open: Arc::new(pacman_open),
            pacman_closed: Arc::new(pacman_closed),
            food: Arc::new(food),
            empty: Arc::new(empty),
        }
    }
...
impl DisplayManager {
    ...
    pub async fn update_display(
        ...
        // Update previous pacman position if it changed
    ...
    fn get_pacman_image(&self, game_state: &GameState) -> DynamicImage {
        let base_image = if game_state.pacman_eating {
            &self.assets.pacman_closed
        } else {
            &self.assets.pacman_open
        };

...
async fn load_game_assets() -> Result<GameAssets, Box<dyn std::error::Error>> {
    let pacman_open = open("resources/open.jpg")?;
    let pacman_closed = open("resources/closed.jpg")?;
    let food = open("resources/food.jpg")?;
    let empty = open("resources/empty.jpg")?;

    println!("Game assets loaded successfully");
    Ok(GameAssets::new(pacman_open, pacman_closed, food, empty))
}
                            ...
                            if key < display_key_count {
                                ...
                                if should_update {
                                    let pacman_pos = {
                                        ...
                                        state.pacman_position
                                    };

                                    if let Err(e) = display_manager
                                        .update_food_at_position(&device, key, pacman_pos)
                                        .await
                                    {
...
async fn run_game_loop(
    ...
    loop {
        ...
        let (ate_food, current_position) = {
            let mut state = game_state.lock().await;
            let current_pos = state.pacman_position;
            let ate = state.eat_food(current_pos);
            (ate, current_pos)
        };

        if ate_food {
            println!("Pacman ate food at position {}", current_position);
        ...
        // Move pacman and update display
        {
            let mut state = game_state.lock().await;
            state.move_pacman(display_key_count);
            println!("Pacman moved to position {}", state.pacman_position);
